...
gris = colorgris(ima);
Hh = filtroFreiChen(gris,1);
Hv = filtroFreiChen(gris,2);
imaMod = Hh +Hv;
...
subplot(2,2,4)
imshow(imaMod), title('Imagen con filtro Frei-Chen');%Imágen filtrada 


function filtrado = filtroFreiChen(ima,operador)
%{
...
for i=2:fil-1
    for j=2:col-1
        ...
        for k=1:3
            for m=1:3
             filtrado(i,j) = double(filtrado(i,j)+matriz(k,m)*freiChen(k,m));
            end
        end
    end
end
